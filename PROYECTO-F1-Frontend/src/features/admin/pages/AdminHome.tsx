...
export default function AdminHome() {
  ...
  const sincronizar = async () => {
    ...
    try {
      ...
      setMensaje(
        `${resultado.detail} ${resultado.grandes_premios} GP, ${resultado.escuderias} escuderías y ${resultado.pilotos} pilotos agregados.`,
      );
    } catch (err: unknown) {
      setError(getErrorMessage(err, 'No se pudieron importar los datos.'));
    } finally {
      setCargando(false);
    }
  };
  ...
  return (
    <div className="stack">
      ...
      <Card>
        ...
        <p>Importa manualmente calendario, escuderías y pilotos desde TheSportsDB. Los datos ya editados no se modifican.</p>
        <div className="flex-between" style={{ gap: '0.75rem', marginTop: '1rem' }}>
          <div className="form-group" style={{ minWidth: 150 }}>
            <label htmlFor="temporada-sync">Temporada</label>
            <input id="temporada-sync" type="number" value={temporada} onChange={(e) => setTemporada(Number(e.target.value))} />
          </div>
          <Button onClick={() => void sincronizar()} disabled={cargando}>
            Importar datos
          </Button>
        </div>
